...
def _cell_arrays(path: Path) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    errors: list[float] = []
...
def _inverse_risk_maximum(log_risk: np.ndarray) -> float:
    """Undo the risk model's log1p absolute-error target for diagnostics."""

    return float(np.expm1(np.max(np.asarray(log_risk, dtype=float))))
...
def main() -> None:
    ...
    absolute_error, risk = _cell_arrays(args.cells)
    ...
    ax.set_xlim(0, 100)
    ax.legend(frameon=False)
    maxima = [_inverse_risk_maximum(risk[method]) for method in STYLES]
    ax.text(
        0.98, 0.04,
        "Unclipped max $\\mathrm{expm1}$(log-risk)\n"
        + "spread / support / hybrid: "
        + " / ".join(f"{value:.1e}" for value in maxima),
        transform=ax.transAxes, ha="right", va="bottom", fontsize=7.0,
        color="#46505f", bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "none"},
    )
